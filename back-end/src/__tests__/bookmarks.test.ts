...
describe('Bookmarks API', () => {
  ...
  it('should delete a bookmark', async () => {
    const response = await request(app).delete(`/api/v1/bookmarks/${createdId}`);
    expect([200, 404]).toContain(response.status);
  });

  it('should return not found when deleting a non-existent bookmark', async () => {
    const response = await request(app).delete(`/api/v1/bookmarks/123456789`);
    expect([200, 404]).toContain(response.status);
  });

  it('should return 400 for invalid delete param', async () => {
    const response = await request(app).delete('/api/v1/bookmarks/not-a-number');
    expect(response.status).toBe(400);
  });
});
